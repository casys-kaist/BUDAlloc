...
#define ALIGNMENT                                                                                                      \
	16ul //4ul				///< This is the byte alignment that memory must be allocated on. IMPORTANT for GTK and other stuff.

#define ALIGN_TYPE char ///unsigned char[16] /// unsigned short
#define ALIGN_INFO                                                                                                     \
	sizeof(ALIGN_TYPE) *                                                                                           \
		16 ///< Alignment information is stored right before the pointer. This is the number of bytes of information stored there.
...
{
	return plat_mmap(0, size * 4096, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
}

int liballoc_free(void *addr, size_t size)
{
	return plat_munmap(addr, size);
}